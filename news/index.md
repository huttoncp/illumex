...
- [`ilm_profile()`](https://huttoncp.github.io/illumex/reference/ilm_profile.md)
  ...
  higher: the middle half 64 to 72, against 29 to 54 overall”. The
  dimension-based sentences named which variables a cluster’s position
  was made of without saying which way the cluster lay on them: on a
  sample with three known subgroups, which the clustering recovered
  almost exactly, two of the three got the same sentence word for word.
  Variables are ranked by a v-test against all rows (as FactoMineR’s
  `catdes()`) and named when it clears `vtest_threshold` and the
  difference is big enough to matter, 0.2 standard deviations or 10
  percentage points – on 1,200 rows a column of pure noise cleared 1.96
  in two clusters of three. A date is described in dates, down to the
  stretch of a cycle where a cluster stands out (“falls on Sat-Sun for
  87% of them, against 50% overall”); markers of missingness as “income
  is missing for 92% of them”. The variables that set no cluster apart
