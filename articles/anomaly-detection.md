...
`illume::ilm_impute()` chooses the number of directions by
cross-validating held-out cells, which is exactly right for filling a
value in and wrong here. On a rank-2 structure in eight columns it chose
**6 or 7** – on clean data as well as contaminated – and those extra
components span the very directions the anomalies depart along.
Detection fell from 0.975 to 0.560.
...
`vignette("workflow", package = "illume")` for where this sits.
